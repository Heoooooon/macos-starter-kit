...
## ✨ v0.2.0 하이라이트 — "안전"을 증명하는 릴리스

수십 가지 실사용 시나리오(회사 PC, 권한 없는 계정, 네트워크 장애, 한글 설정 파일…)를
점검해 크게 다듬었습니다. ([전체 릴리스 노트 ↗](https://github.com/Heoooooon/lazy-starter-kit/releases/tag/v0.2.0))

- 🧪 **6개 환경에서 커밋마다 진짜로 설치해봅니다** — macOS · Windows · Ubuntu ·
  Fedora · Arch · openSUSE에서 설치→검증→제거 전 과정을 CI가 자동 반복.
  "내 환경에서 되나?"를 문서가 아니라 테스트가 보증해요.
- 🛟 **설정 파일 이중 안전장치** — `~/.zshrc`/PowerShell 프로필을 처음 고치기 전
  **자동 백업**(`.bak`)을 만들고, 마커가 손상된 파일은 **수정을 거부**합니다.
- 🏢 **회사 PC에서 더 강해짐** — 관리자 권한(sudo) 없어도 중단 없이 유저 도구는
  끝까지 설치, Windows PowerShell 5.1의 설치 중단 버그들 해결, **한국어 프로필
  인코딩 보존**.
- 🔍 **공급망 투명성** — 외부 설치 스크립트 핀·검증, [SECURITY.md](./SECURITY.md),
  GitHub Actions SHA 고정. 뭘 믿고 실행하는지 숨기지 않습니다.
...
```sh
STARTER_KIT_BRANCH=v0.2.0 bash -c "$(curl -fsSL https://raw.githubusercontent.com/Heoooooon/lazy-starter-kit/v0.2.0/install.sh)"
```
